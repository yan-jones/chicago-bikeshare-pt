...
answer = "Porque existem valores vazios (Missing)"
...
trip_duration_list = [float(data[2]) for data in data_list]
trip_duration_list = sorted(trip_duration_list, key=float)
min_trip = trip_duration_list[0]
max_trip = trip_duration_list[-1]
mean_trip = sum(trip_duration_list)/len(trip_duration_list)
qtd = len(trip_duration_list)
if(qtd % 2 != 0):
    median_trip = trip_duration_list[qtd//2]
else:
    median_trip = sum(trip_duration_list[qtd//2-1:qtd//2+1])/2.0
...
start_stations = set([data[3] for data in data_list])
